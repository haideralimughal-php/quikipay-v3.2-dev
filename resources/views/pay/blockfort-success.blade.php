...
<div class="card QRCodeAnimationClass m-auto d-block pb-2 pt-2  box-shadow QR-card"  >
	...
	<div class="card-body" style="padding: 1.25rem;padding-top:0; ">
		...
		<h3 class="text-center mt-3 mb-3"><b>{{ number_format($transaction->quantity) }} {{ $transaction->currency_symbol }}</b></h3>
	
		<hr>
		<div class="card-text">
			<div class="row">
				<div class="col"><p class="float-left text-primary">{{__('text.amountPaid')}}</p></div>
				<div class="col"><p class="float-right text-primary   ">{{ number_format($transaction->quantity) }} {{ $transaction->currency_symbol }}  </p></div>
			</div>
		</div>
	</div>
    <div class="mt-2 mb-5">  
    	...
    	@if(session()->has('site_url'))
    	    <form action="{{ session('redirect') }}" method="post" id="transaction_form">
        	    ...
        	    <input type="hidden" name="payment_method" value="KHIPU">
